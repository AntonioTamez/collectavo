...
- **Global scope**: `C:/ATS/Git/collectavo/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/ATS/Git/collectavo/.claude/notes/`)
...
2. Glob `C:/ATS/Git/collectavo/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/ATS/Git/collectavo/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/ATS/Git/collectavo/.claude/notes/` — works in any directory
